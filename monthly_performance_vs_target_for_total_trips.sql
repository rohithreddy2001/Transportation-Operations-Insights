create table monthly_performance_vs_target_for_total_trips as (
SELECT 
    city.city_name,
    ...
    CASE
        WHEN COUNT(trips.trip_id) > ROUND(AVG(target_trips.total_target_trips), 0) THEN 'Target Exceeded'
        WHEN COUNT(trips.trip_id) < ROUND(AVG(target_trips.total_target_trips), 0) THEN 'Target Missed'
        ELSE 'Target Met'
